...
class Request:
	...
	def return_requests(self):
		...
		if p is None:
			return []
		return (p,of,STREAM_PIECE_SIZE,s)

class Peer:
	def __init__(self,content_id,handshake,Container,Buffer,ip=None,port=None,sock=None,server=False):
	...
	def handle_connect(self,sock=None,ip=None,port=None):
		...
		if sock is not None:
			...
			self.ip = self.to_bytes('!B',sock.getpeername()[0])
			self.raw_ip = sock.getpeername()[0]
			# it's None because we don't know peer's port (on which he will accept connections) -> it will be
			# known after handshaking
			self.port = None
			self.raw_port = None
	...
	def handle_write(self):
		...
		while self.streamer:
			...
			# type(data) == int => no piece in Buffer at this moment
			break
		t = time.time()
		if self.send_buffer:
			try:
		...
		while self.read_buffer:
				...
				if (msg[:12]).lower() == 'salamatsyzby':
					if msg[12:44] == self.content_id:
						if not self.initiated:
							self.send_buffer += self.handshake
							self.port = msg[64:66]
							self.raw_port = struct.unpack('!H',self.port)[0]
							self.C.add_new_peer(self.ip,self.port)
						self.C.connected_peers += 1
						self.handshaked = True
						self.logger.debug('Connection with peer ' + (self.raw_ip + ':' + str(self.raw_port)) + ' established')
						continue
					else:
						try:self.socket.send(struct.pack('!I',19) + ERROR + pickle.dumps('Invalid Content ID'))
						except: pass
						self.logger.debug('Connection with peer not established --> ' + 'recieved Invalid CONTENT ID')
						return self.handle_close()
				if msg[0] == ERROR:
					try:
			...
			if id == KEEP_ALIVE:
				...
				try:
					...
					pass
				self.logger('Stopped streaming ' + str((pos,offset,length,seconds)))
			elif id == HAVE:
...
def test_read():
	...
	b = BuffeR()
	s = sock()
	p = Peer('content_id','handshake',c,b,sock=s)
	...
	s = sock()
	p = Peer('content_id1234567890123456789012','handshake',c,b,sock=s)
	s.r_buf = [struct.pack('!I',66)+'Salamatsyzby' + 'content_id1234567890123456789012' + 'peeeeeerrrrrr_iiiidd' + '12','except']
	p.handle_read()
	assert not s.closed
	assert p.send_buffer == 'handshake'
	assert p.port == '12'
	assert p.handshaked
	s = sock()
	p = Peer('content_id1234567890123456789012','handshake',c,b,sock=s)
	p.initiated = True
	s.r_buf = [struct.pack('!I',66)+'Salamatsyzby' + 'content_id1234567890123456789012' + 'peeeeeerrrrrr_iiiidd12',\
				struct.pack('!I',2)+'qw' ,'except']
	p.handle_read()
	assert s.closed
	...
	s = sock()
	p = Peer('content_id1234567890123456789012','handshake',c,b,sock=s)
	m = pickle.dumps('Invalid Content ID')
	s.r_buf = [struct.pack('!I',1+len(m)) + ERROR + m,'except']
	p.handle_read()
	assert s.closed
	c.deleted = False
	s = sock()
	p = Peer('content_id1234567890123456789012','handshake',c,b,sock=s)
	m = pickle.dumps('Another Error')
	s.r_buf = [struct.pack('!I',1+len(m)) + ERROR + m,'except']
	p.handle_read()
	assert s.closed
	...
	p = Peer('content_id1234567890123456789012','handshake',c,b,sock=s)
	p.initiated = True
	s.r_buf = [struct.pack('!I',66) + 'Salamatsyzby' + 'content_id1234567890123456789012' + 'peeeeeeerrrrr_iiiidd' + '12',\
				struct.pack('!I',1) + POSITION, struct.pack('!I',3) + POSITION + struct.pack('!H',128), 'except']
	b.pos = '1'
	p.handle_read()
	assert c.start_pos == 128
	assert not s.closed
	assert p.send_buffer[5] == '1'
	assert not p.streamer
	p.send_buffer = ''
	p.requested[(1,0,15,2)] = {}
	...
	assert len(p.requested[(1,0,15,2)]) == 2
	assert not p.store.have(23)
	c.connected_peers = 2
	s.r_buf = [struct.pack('!I',1) + STOP, struct.pack('!I',5) + HAVE + struct.pack('!I',23),struct.pack('!I',1) + CLOSE, 'except']
	p.handle_read()
	assert not p.streamer
...
def test_put():
	...
	p.requested[(2,0,15,2)] = {}
	p.active = True
	p.put(2,0, struct.pack('!I',15) + 'data')
	p.put(4,0,'data4')
	assert len(p.requested[(2,0,15,2)]) == 1
	p.put(2,4,'datadata')
	assert not p.requested[(2,0,15,2)][2]['completed']
	p.put(2,12,'dat')
	assert p.requested[(2,0,15,2)][2]['data'].len == 15
	assert p.requested[(2,0,15,2)][2]['completed']
...
def test_return():
	...
	s = sock()
	p = Peer('content_id','handshake',c,b,sock=s)
	p.requested = {(1,0,15,3):{}}
	assert p.return_requests() == [(1,0,15,3)]
	class data:
		def __init__(self,l):
			self.len = l
	p.requested = {(1,0,15,3):{1:{'completed':False,'data':data(10)}}}
	assert p.return_requests() == [(1,10,15,1),(2,0,15,2)]
	p.requested = {(1,0,15,3):{1:{'completed':True,'data':data(10)},2:{'completed':False,'data':data(8)}}}
	assert p.return_requests() == [(2,8,15,1),(3,0,15,1)]
	p.requested = {}
	assert p.return_requests() == []
	p.requested = {(1,0,15,1):{1:{'completed':False,'data':data(10)}}}
	assert p.return_requests() == [(1,10,15,1)]
#
#def test_timeout():
#	pass
#
def test():
	...
	p.handshaked = True
	p.request_peers()
	assert p.send_buffer == struct.pack('!I',2) + GET_PEERS + struct.pack('!B',35)
	p.send_buffer = ''
	p.request_position()
