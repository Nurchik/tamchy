...
class StreamContainer(threading.Thread):
	def __init__(self,info,peer_id,port,max_connections=50,is_server=False,source='',bitrate=0,ext_ip=''):
		...
		self.R = Reactor(Server(self,port=port))
		self.B = StreamBuffer(self.content_id)
		self.work = True
	...
	def prepare_peer(self,ip=None,port=None,sock=None):
		if sock is None:
			...
			p = Peer(self.content_id,self.handshake,self,self.B,sock=sock)
		self.add(p)
				...
				for i in xrange(COUNTER):
					...
					while (time() - t) < 1:
						try : 
							...
							self.logger.error('Cannot fetch data from source COUNTER --> ' + str(i))
							raise Exception
